...
codriver is a driving overlay for the Tesla in-car browser. The map renders incidents (police, accidents, hazards, road closures, jams), speed cameras and chargers as pins over a 3D basemap, with turn-by-turn navigation. Every pin comes from a **feed** — a source of data published in codriver's canonical entity format. codriver owns the schema, the kind taxonomy, the icons and the decay rules; feeds fill in the data.

This site is the developer reference for publishing one. Read it if you want codriver to render your traffic data, charger network, snow-removal feed, hazard reports — anything that fits a `(lat, lng, kind, properties)` shape.
